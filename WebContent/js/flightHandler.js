...
function displayOneWayNonstop(t){
	for(var i in t){
		$(".flight_container").append('<div class="flight_result flight_oneway"> '
							...
							+'				<span>&#163; </span>'+generateFlightPrice()+' </span>'
							+'			</div>'
							+'		</div>  '
							+'	</div>  '
							...
							+'						<div class="ItineraryLeg__segment-time-3IWQh">  '
							...
							+'			</span>  '
							+'		</div>  '
							+'		<div class="dropdown-grids search_grid" role="button" onclick="redirect(this);">  '
	...
	for (var i in ticket1){
		for (var j in ticket2){
			...
			$('.flight_container').append(' <div class="flight_result flight_oneway_return">'
					...
					+'				<span>&#163; </span>'+(oneway_price + return_price)+' </span>'
					+'			</div>'
					+'		</div>'
					+'	</div> ');
		}
